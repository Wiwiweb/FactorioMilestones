...
local function get_timestamp(ticks, print_milliseconds)
    if print_milliseconds then
        ...
        return format_time(ticks) .. "." .. string.format("%03d", milliseconds)
    else
...
function enable_edit_time(player_index, element)
    ...
    local default_value = format_time(ceil_to_nearest_minute(milestone.completion_tick))
...
function build_display_page(player)
    ...
    local display_scroll = inner_frame.add{type="scroll-pane", name="milestones_display_scroll", style="milestones_naked_scroll_pane"}
